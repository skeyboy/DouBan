...
public class HTTPTask : NSObject, NSURLSessionDelegate, NSURLSessionTaskDelegate,NSURLSessionDataDelegate {
    ...
    */
    ...
    func httpQueue(url: String, method: HTTPMethod, parameters: Dictionary<String,AnyObject>!, success:((HTTPResponse) -> Void)!, failure:((NSError) -> Void)!){
        var task = HTTPTask()
        Singleton.queue.addOperation(task.create(url, method: method, parameters: parameters, success: success, failure: failure)!)
    }
